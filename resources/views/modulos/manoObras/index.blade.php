    ...
    <div class="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div class="bg-base-100 p-6 rounded-xl shadow-lg">
            <div class="flex justify-between items-center mb-4">
                <div>
                    <h1 class="text-2xl font-bold text-base-content">Trabajadores</h1>
                    <p class="text-sm text-base-content/70">Lista de trabajadores</p>
                </div>
                <a href="{{ route('manoObras.create') }}" class="btn btn-primary btn-md gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="table table-zebra w-full text-sm">
                    ...
                    <tbody>
                        @forelse ($manoObras as $manoObra)
                            <tr class="hover:bg-base-200">
                                <td>{{ $manoObra->id }}</td>
                                <td>{{ $manoObra->nombre }}</td>
                                <td>{{ $manoObra->cedula }}</td>
                                <td>
                                    @if ($manoObra->imagen)
                                        <div class="group w-16 h-16 relative overflow-visible">
                                            <img src="{{ asset('storage/' . $manoObra->imagen) }}"
                                ...
                                <td class="flex gap-2">
                                    {{-- Ver --}}
                                    <button onclick="showModal({{ $manoObra->id }})"
                                    ...
                                    {{-- Editar --}}
                                    <a href="{{ route('manoObras.edit', $manoObra->id) }}"
                                    ...
                                    {{-- Eliminar --}}
                                    <button onclick="showDeleteModal({{ $manoObra->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-6 text-base-content/50">No hay Trabajadores
                                    registrados.</td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @section('scripts')
        <script>
            ...
            function showDeleteModal(id) {
                ...
                form.action = `/manoObras/${id}/destroy`;
                modal.showModal();
            }
        </script>
    @endsection
