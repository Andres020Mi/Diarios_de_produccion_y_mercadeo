    ...
    <div class="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div class="bg-base-100 p-6 rounded-xl shadow-lg">
            <div class="flex justify-between items-center mb-4">
                <div>
                    <h1 class="text-2xl font-bold text-base-content">Gestores de areas</h1>
                    <p class="text-sm text-base-content/70">Lista de los gestores de areas</p>
                </div>
                <a href="{{ route('gestoresAreas.create') }}" class="btn btn-primary btn-md gap-2">
                ...
                <table class="table table-zebra w-full text-sm">
                    <thead>
                        <tr class="text-base-content/80">
                            ...
                            <td class="text-left">Area de produccion</td>
                            <td class="text-left">Descripcion de area</td>
                            <th class="text-left">Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($gestoresAreas as $unidad)
                            <tr class="hover:bg-base-200">
                                <td>{{ $unidad->id }}</td>
                                <td>{{ $unidad->user->name }}</td>
                                 <td>{{ $unidad->user->email }}</td>
                              
                             
                                    <td>{{ $unidad->areaProduccion->nombre }}</td>
                                      <td class="max-w-xs whitespace-normal break-words text-sm text-base-content">
                                    {{ $unidad->areaProduccion->descripcion }}
                                </td>
                                <td class="flex gap-2">
                                    {{-- Ver --}}
                                    <button onclick="showModal({{ $unidad->id }})"
                                    ...
                                    {{-- Editar --}}
                                    <a href="{{ route('gestoresAreas.edit', $unidad->id) }}"
                                    ...
                                    {{-- Eliminar --}}
                                    <button onclick="showDeleteModal({{ $unidad->id }})"
    ...
    @section('scripts')
        <script>
            ...
            function showDeleteModal(id) {
                ...
                form.action = `/gestoresAreas/${id}/destroy`;
                modal.showModal();
            }
        </script>
    @endsection
