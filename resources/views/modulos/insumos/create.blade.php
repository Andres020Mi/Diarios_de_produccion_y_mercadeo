        ...
        <div class="card bg-base-100 shadow-xl">
            <div class="card-body">
                <div class="flex justify-between items-center mb-4">
                    <div>
                        <h1 class="card-title text-2xl">Crear Insumo</h1>
                        <p class="text-sm text-base-content/70">Formulario para registrar un nuevo insumo</p>
                    </div>
                    <a href="{{ route('insumos.index') }}" class="btn btn-secondary btn-md gap-2">
                ...
                <form action="{{ route('insumos.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-control mb-4">
                        <label class="label block" for="unidad_de_medida">
                            <span class="label-text font-medium">Unidad de Medida *</span>
                        </label>
                        <select id="unidad_de_medida" name="unidad_de_medida" class="select select-bordered w-full max-w-md" required>
                            <option value="">Seleccione una opción</option>
                            <option value="g" {{ old('unidad_de_medida') == 'g' ? 'selected' : '' }}>g</option>
                            <option value="kg" {{ old('unidad_de_medida') == 'kg' ? 'selected' : '' }}>kg</option>
                            <option value="ml" {{ old('unidad_de_medida') == 'ml' ? 'selected' : '' }}>ml</option>
                            <option value="L" {{ old('unidad_de_medida') == 'L' ? 'selected' : '' }}>L</option>
                            <option value="x1" {{ old('unidad_de_medida') == 'x1' ? 'selected' : '' }}>x1</option>
                            <option value="x12" {{ old('unidad_de_medida') == 'x12' ? 'selected' : '' }}>x12</option>
                        </select>
                    </div>

                    <div class="form-control mb-4">
                        <label class="label block" for="imagen">
                            <span class="label-text font-medium">Imagen</span>
                        </label>
