    ...
    <div class="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div class="bg-base-100 p-6 rounded-xl shadow-lg">
            <div class="flex justify-between items-center mb-4">
                <div>
                    <h1 class="text-2xl font-bold text-base-content">Áreas de Producción</h1>
                    <p class="text-sm text-base-content/70">Gestión de registros de áreas de producción existentes</p>
                </div>
                <a href="{{ route('areasProduccions.create') }}" class="btn btn-primary btn-md gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="table table-zebra w-full text-sm">
                    ...
                    <tbody>
                        @forelse ($areasProduccions as $areasProduccion)
                            <tr class="hover:bg-base-200">
                                <td>{{ $areasProduccion->id }}</td>
                                <td>{{ $areasProduccion->nombre }}</td>
                                <td class="max-w-xs whitespace-normal break-words text-sm text-base-content">
                                    {{ $areasProduccion->descripcion }}
                                </td>
                                <td class="flex gap-2">
                                    {{-- Ver --}}
                                    <button onclick="showModal({{ $areasProduccion->id }})"
                                        class="btn btn-sm btn-info btn-square tooltip" data-tip="Ver área">
                                    ...
                                    {{-- Editar --}}
                                    <a href="{{ route('areasProduccions.edit', $areasProduccion->id) }}"
                                        class="btn btn-sm btn-warning btn-square tooltip" data-tip="Editar área">
                                    ...
                                    {{-- Eliminar --}}
                                    <button onclick="showDeleteModal({{ $areasProduccion->id }})"
                                        class="btn btn-sm btn-error btn-square tooltip" data-tip="Eliminar área">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center py-6 text-base-content/50">No hay áreas de producción registradas.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <dialog id="modal-delete" class="modal">
      <div class="modal-box max-w-md bg-base-100 rounded-xl shadow-lg">
        <div class="flex items-center gap-3 mb-4">
            ...
            <h3 class="font-bold text-lg text-base-content">¿Eliminar Área?</h3>
        </div>
        <p class="text-base-content/80">¿Estás seguro de que deseas eliminar esta área de producción? Esta acción no se puede deshacer.</p>
    ...
    @section('scripts')
        <script>
                ...
                fetch(`/areasProduccions/${id}`)
                    ...
                    .then(data => {
                        modalContent.innerHTML = `
                            <div class="space-y-2">
                                <h2 class="text-xl font-bold">${data.nombre}</h2>
                                <p><span class="font-semibold">Descripción:</span> ${data.descripcion}</p>
                                <p><span class="font-semibold">ID:</span> ${data.id}</p>
                            </div>
                            <div class="card-actions justify-end mt-4">
                                <button class="btn btn-primary" onclick="document.getElementById('modal-area').close()">Cerrar</button>
                            </div>
                        `;
                        modal.showModal();
                    })
                    .catch(error => {
                        ...
                        modalContent.innerHTML =
                            `<p class="text-error font-semibold">Error al cargar los datos del área.</p>`;
                        modal.showModal();
                    });
            }
            ...
            function showDeleteModal(id) {
                ...
                form.action = `/areasProduccions/${id}/destroy`;
                modal.showModal();
            }
        </script>
    @endsection
