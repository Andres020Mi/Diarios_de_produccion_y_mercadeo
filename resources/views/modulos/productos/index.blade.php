...
@section('content')
    ...
    <div class="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div class="bg-base-100 p-6 rounded-xl shadow-lg">
            <div class="flex justify-between items-center mb-4">
                <div>
                    <h1 class="text-2xl font-bold text-base-content">Productos</h1>
                    <p class="text-sm text-base-content/70">Lista de registros de Productos existentes</p>
                </div>
                <a href="{{ route('productos.create') }}" class="btn btn-primary btn-md gap-2">
                ...
                <table class="table table-zebra w-full text-sm">
                    <thead>
                        <tr class="text-base-content/80">
                            ...
                            <th class="text-left">Unidad de medida produccion</th>
                            <th class="text-left">Unidad de medida mercadeo</th>
                            <th class="text-left">Precio por unidad</th>
                    ...
                    <tbody>
                        @forelse ($productos as $producto)
                            <tr class="hover:bg-base-200">
                                ...
                                <td class="max-w-xs whitespace-normal break-words text-sm text-base-content">
                                    {{ $producto->unidad_de_medida_produccion }}
                                </td>
                                <td class="max-w-xs whitespace-normal break-words text-sm text-base-content">
                                    {{ $producto->unidad_de_medida_mercadeo }}
                                </td>
                                   <td class="max-w-xs whitespace-normal break-words text-sm text-base-content">
                                ...
                                <td>
                                    @if ($producto->imagen)
                                        <div class="group w-16 h-16 relative overflow-visible">
                                            <img src="{{ asset('storage/' . $producto->imagen) }}"
                                                class="w-full h-full object-cover rounded-md transition-transform duration-300 ease-in-out group-hover:scale-150 absolute left-0 top-0 z-10 bg-transparent"
                                                style="transform-origin: center;" alt="Imagen de insumo" />
                                        </div>
                                    @else
                                        <span class="text-sm text-base-content/50">Sin imagen</span>
                                    @endif
                                </td>
                                <td class="flex gap-2">
                                    {{-- Ver --}}
                                    <button onclick="showModal({{ $producto->id }})"
                                    ...
                                    {{-- Eliminar --}}
                                    <button onclick="showDeleteModal({{ $producto->id }})"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-6 text-base-content/50">No hay Productos
                                    registrados.</td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <dialog id="modal-insumo" class="modal">
        <div class="modal-box max-w-lg flex flex-col max-h-[80vh] bg-base-100 rounded-xl shadow-lg">
            <div class="flex justify-between items-center mb-4">
                <h3 class="font-bold text-lg text-base-content">Detalles del Producto</h3>
                <form method="dialog">
    ...
    <dialog id="modal-delete" class="modal">
        <div class="modal-box max-w-md bg-base-100 rounded-xl shadow-lg">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="font-bold text-lg text-base-content">¿Eliminar Producto?</h3>
            </div>
            <p class="text-base-content/80">¿Estás seguro de que deseas eliminar este Producto? Esta acción no se puede
                deshacer.</p>
        ...
        <script>
            ...
            `;
                    ...
                    .catch(error => {
                        ...
                        modalContent.innerHTML =
                            `<p class="text-error font-semibold">Error al cargar los datos del insumo.</p>`;
                        modal.showModal();
                    });
            }
